...
ggplotly(ggplot(combotabel3_ZP, aes(Tijdstip, as.factor(Diepte), fill = Temperatuur)) +
  ...
  labs(title = "Thermische stratificatie Nieuwkapelleplas", x = "Tijdstip", y = "Diepte (m)", fill = "Temperatuur") +
  theme_minimal() +
...
ggplotly(ggplot(combotabel3_ZP, aes(Tijdstip, as.factor(Diepte), fill = log10(Helderheid))) +
  ...
  labs(title = "Temperatuursverdeling Nieuwkapelleplas", x = "Tijdstip", y = "Diepte (m)", fill = "Helderheid") + scale_y_reverse(breaks = seq(0,8,0.5)) +
  theme_minimal() +
  scale_y_discrete(limits = rev) +
  scale_x_datetime(breaks = date_breaks("1 month"), labels = date_format("%b %Y"))
)
