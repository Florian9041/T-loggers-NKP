...
```{r}
est_temp_by_date <- function(target_date, target_depth) {
  data_for_date <- combotabel3_ZP %>%
    filter(date == target_date) %>%
    arrange(depth)
approx(data_for_date$depth, data_for_date$temp, xout = target_depth)$y
}

est_temp_by_date(, c(0,1,1.5,2))
...
```{r}
afgeronde_data <- combotabel3_ZP[,c(2,3,6)] %>%
  mutate(across(c(Temperatuur, Diepte), round, digits = 2))
afgeronde_data$Tijdstip <- as.POSIXct(afgeronde_data$Tijdstip, format = "%Y-%m-%d %H:%M:%S")
  
ggplot(afgeronde_data, aes(Tijdstip, Diepte, fill = Temperatuur)) +
  geom_tile() +
...
complete_data$Temperatuur <- na.approx(complete_data$Temperatuur,
          ...
          na.rm = FALSE)
# plot opnieuw
ggplot(complete_data, aes(Tijdstip, Diepte, fill = Temperatuur)) +
  geom_tile() +
...
```{r}
library(fields)
image.plot(afgeronde_data$Tijdstip, afgeronde_data$Diepte, afgeronde_data$Temperatuur)
```
