...
# Specify the date to remove (2024-04-22)
date_to_remove1 <- as.Date("2024-04-22")
date_to_remove2 <- as.Date("2023-10-31")

# Filter out all data for 2024-04-22 (all hours)
combotabel3_ZP <- combotabel3_ZP %>%
  filter(as.Date(Tijdstip) != date_to_remove1)
combotabel3_ZP <- combotabel3_ZP %>%
  filter(as.Date(Tijdstip) != date_to_remove2)
...
ggplot(combotabel3_ZP, aes(Tijdstip, as.factor(Diepte), fill = Temperatuur)) +
  ...
  labs(title = "Thermische stratificatie Nieuwkapelleplas", x = "Tijdstip", y = "Diepte (m)", fill = "Temperatuur (°C)") +
  theme_classic2() +
...
```{r}
# Lux hoger dan 15000
combotabel3_ZP_filtered_high <- combotabel3_ZP[combotabel3_ZP$Helderheid > 1000, ]
...
ggplot(combotabel3_ZP, aes(Tijdstip, as.factor(Diepte), fill = log10(Helderheid))) +
  ...
  labs(title = "Temperatuursverdeling Nieuwkapelleplas", x = "Tijdstip", y = "Diepte (m)", fill = "Helderheid (log10 lux)") + scale_y_reverse(breaks = seq(0,8,0.5)) +
  theme_linedraw() +
  scale_y_discrete(limits = rev) +
  scale_x_datetime(breaks = date_breaks("1 month"), labels = date_format("%b %Y")) +
  geom_point(data = combotabel3_ZP_filtered_high, aes(Tijdstip, as.factor(Diepte)), color = "green4", size = 2)  # Add points for Helderheid > 1000
```

```{r}
# grootste diepte per week
# Filter for Helderheid > 1000
combotabel3_ZP_filtered_high <- combotabel3_ZP[combotabel3_ZP$Helderheid > 1000, ]

# Convert 'Tijdstip' to Date or POSIXct format (if not already)
combotabel3_ZP_filtered_high$Tijdstip <- as.POSIXct(combotabel3_ZP_filtered_high$Tijdstip)

# Add a new column for the week of the year
combotabel3_ZP_filtered_high <- combotabel3_ZP_filtered_high %>%
  mutate(week = week(Tijdstip), year = year(Tijdstip))

# Select the highest Diepte per week
combotabel3_ZP_max_depth_per_week <- combotabel3_ZP_filtered_high %>%
  group_by(year, week) %>%
  filter(Diepte == max(Diepte)) %>%
  ungroup()  # Ungroup after filtering

combotabel3_ZP_max_depth_per_week <- combotabel3_ZP_max_depth_per_week %>%
  mutate(lux_1000 = ifelse(Helderheid > 1000, "lux > 1000", "lux <= 1000"))
...
ggplot(combotabel3_ZP, aes(Tijdstip, as.factor(Diepte), fill = log10(Helderheid))) +
  geom_tile() +
  scale_fill_gradient2(low = "black", high = "red", midpoint = 1, na.value = "black") +
  labs(title = "Temperatuursverdeling Nieuwkapelleplas", x = "Tijdstip", y = "Diepte (m)", fill = "Helderheid (log10 lux)") +
  scale_y_reverse(breaks = seq(0, 8, 0.5)) +
  theme_linedraw() +
  scale_y_discrete(limits = rev) +
  scale_x_datetime(breaks = date_breaks("1 month"), labels = date_format("%b %Y")) +
  geom_point(data = combotabel3_ZP_max_depth_per_week, aes(Tijdstip, as.factor(Diepte), colour = lux_1000), size = 2) +
  scale_color_manual(values = c("lux > 1000" = "green4", "lux <= 1000" = "transparent")) +  # Set color for legend
  guides(color = guide_legend(title = "Plantengroei"))  # Customize legend title
...
# Convert 'Tijdstip' to POSIXct format (if not already)
combotabel3_ZP_filtered_high$Tijdstip <- as.POSIXct(combotabel3_ZP_filtered_high$Tijdstip)

# Add a new column for the date part of 'Tijdstip'
combotabel3_ZP_filtered_high <- combotabel3_ZP_filtered_high %>%
  mutate(date = as.Date(Tijdstip))

# Select the highest Diepte per day
combotabel3_ZP_max_depth_per_day <- combotabel3_ZP_filtered_high %>%
  group_by(date) %>%
  filter(Diepte == max(Diepte)) %>%
  ungroup()  # Ungroup after filtering

# Add a column to indicate whether Helderheid is greater than 1000 (for legend)
combotabel3_ZP_max_depth_per_day <- combotabel3_ZP_max_depth_per_day %>%
  mutate(lux_1000 = ifelse(Helderheid > 1000, "lux > 1000", "lux <= 1000"))
...
ggplot(combotabel3_ZP, aes(Tijdstip, as.factor(Diepte), fill = log10(Helderheid))) +
  ...
  scale_y_discrete(limits = rev) +
  scale_x_datetime(breaks = date_breaks("1 month"), labels = date_format("%b %Y")) +
  geom_point(data = combotabel3_ZP_max_depth_per_day, aes(Tijdstip, as.factor(Diepte), color = lux_1000), size = 2) +  # Add points for max Diepte per day
  scale_color_manual(values = c("lux > 1000" = "green4", "lux <= 1000" = "transparent")) +  # Set color for legend
  guides(color = guide_legend(title = "Plantengroei"))  # Customize legend title
...
# LakeEnsembleR
```{r}
# Alleen data zonnepaneelzone nodig
