...
```{r}
# Define the range of date-time you want to complete (from min to max Tijdstip)
start_time <- min(combotabel3_ZP$Tijdstip)
end_time <- max(combotabel3_ZP$Tijdstip)

# Create a complete sequence of date-time values (hourly in this case)
complete_times <- seq(from = start_time, to = end_time, by = "hour")  # Change "hour" to "minute" if needed

# Create a data frame with the complete date-time sequence
complete_times_df <- data.frame(Tijdstip = complete_times)

# Merge the original dataset with the complete date-time sequence
combotabel3_ZP_complete <- merge(complete_times_df, combotabel3_ZP, by = "Tijdstip", all.x = TRUE)

# Check the result
head(combotabel3_ZP_complete)
