...
scale_x_datetime(breaks = date_breaks("1 month"), labels = date_format("%b %Y")) +
  theme(
    ...
    axis.text = element_text(size = 12)) # Remove axis titles
...
ggplot(combotabel3_ZP, aes(Tijdstip, as.factor(Diepte), fill = log10(Helderheid))) +
  ...
  geom_point(data = combotabel3_ZP_max_depth_per_week_combined, aes(Tijdstip, as.factor(Diepte), colour = lux_condition), size = 3) +
  scale_color_manual(values = c("lux > 1000" = "green4", "lux > 500" = "blue")) +  # Color for both conditions
  guides(color = guide_legend(title = "Helderheid")) +
  theme(
    plot.title = element_blank(),  # Remove the main title
    axis.title = element_blank(),  # Remove axis titles
    axis.text = element_text(size = 10)  # Optional: Adjust axis text size
  )
